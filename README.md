...
- Reasoner, worker, and retrieval pools with explicit, many-to-many GPU assignment — one card can serve every pool at once.
...
- **Administration** groups access control, execution policy, email, statistics, models, and hardware into six independent domains, each its own sub-page reachable from an expandable submenu nested under Administration in the sidebar. Every domain is gated by its own permission, so a role only ever sees the domains it can act on; a role with exactly one accessible domain lands on it directly, without a submenu to expand. The hardware domain retains the latest 15 minutes of measurements and draws power, GPU load, VRAM use, and temperature charts for every configured pool, labelled with its domain. The Administration entry itself is rendered only when at least one domain is accessible.
...
Once a weight is registered it stays in the **Model registry** list, whose file size, context, and parameter facts would otherwise disappear along with the file explorer. Skein reads that information from the GGUF file's own header: file size from a live `stat()`, and architecture, trained context length, and an exact parameter count summed from the file's tensor shapes (most GGUF files never carry a `general.parameter_count` field, so the metadata alone cannot give a total) — parsed once per model, cached, and served from the database from then on. For MoE architectures that also declare `expert_count`/`expert_used_count`, Skein separates active parameters (shared tensors plus the used fraction of the expert pool) from the total; dense models show one figure. Any of these facts that a given file's header does not carry is labelled unavailable rather than guessed.
...
The Execution view reports live runtime state instead of static domain labels. For both reasoner and worker it shows endpoint health, active tasks versus shared task capacity, queued tasks, recent average tokens per second, and recent average execution time, plus a compact power/GPU-load chart for that role's pool so a live health check never requires switching to the Hardware page. The recap also shows active and queued workflows, current GPU power from `nvidia-smi`, CPU utilization, and used/total RAM. CPU package and RAM watts are read from LibreHardwareMonitor/OpenHardwareMonitor WMI sensors when available. Without those sensors, Skein displays clearly marked estimates: CPU load applied to a logical-core power envelope and used RAM at 0.375 W/GB. Pool charts only contain GPU telemetry after one or more GPUs have been assigned to that pool; unavailable readings are labelled explicitly instead of being fabricated. A GPU can be assigned to several pools at once — the common single-GPU setup runs reasoner, worker, and retrieval on the same card — in which case its full telemetry is reported under each assigned pool rather than split between them, the same shared-attribution principle already used for per-task energy estimates. A pool that has running models but no assigned GPU shows an explicit warning instead of a silent zero.

Each pool card on the Hardware page is bordered in that pool's own color, since a shared GPU reports identical numbers under every pool it belongs to and the four metric colors stay fixed across pools for easy comparison — the border is what tells Reasoner, Workers, and Retrieval apart at a glance. Each card also lists the model(s) currently configured for that pool with their runtime status, so the numbers are never anonymous.

Splitting a shared GPU's actual usage between the models running on it is not measurable on this stack: `nvidia-smi` exposes no per-process utilization or memory on Windows GeForce (WDDM) drivers, unlike Linux datacenter cards in TCC mode. Each GPU card instead shows an explicitly labelled **estimate** of VRAM by model, computed from the GGUF file size (the weights fully offloaded with `-ngl 999`) plus roughly 3% of that size per 4096 tokens of configured context as a rough KV-cache proxy, plus a fixed 350 MB compute-buffer allowance. Actual usage varies by architecture, batch size, and quantization; power and compute load are not split per model at all, since no comparable estimation basis exists for either.
