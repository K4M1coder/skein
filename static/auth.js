(() => {
  ...
  const renderAdmin = async session => {
    ...
    if(can("email.manage")){
      ...
      document.querySelector("#smtp-settings").innerHTML=`<form id="smtp-form" class="smtp-form"><input name="host" placeholder="${t("smtpHost")}" value="${config.host||""}" required><input name="port" type="number" value="${config.port||587}" required><select name="security"><option value="starttls" ${config.security==="starttls"?"selected":""}>STARTTLS</option><option value="ssl" ${config.security==="ssl"?"selected":""}>SSL/TLS</option><option value="plain" ${config.security==="plain"?"selected":""}>${t("plain")}</option></select><input name="username" placeholder="${t("smtpUsername")}" value="${config.username||""}"><input name="password" type="password" placeholder="${t("smtpPassword")}"><input name="from_address" type="email" placeholder="${t("senderAddress")}" value="${config.from_address||""}" required><button>${t("saveSmtp")}</button></form><form id="smtp-test" class="smtp-test"><input name="recipient" type="email" placeholder="${t("testRecipient")}" required><button>${t("sendTestEmail")}</button></form><div id="smtp-status"></div>`;
  ...
  const boot = async () => {
    try {
      ...
      const script=document.createElement("script"); script.src="/app.js?v=19"; document.body.appendChild(script);
