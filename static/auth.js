(() => {
  ...
  const showLogin = () => {
    document.body.insertAdjacentHTML("beforeend", `<div class="auth-overlay"><div class="auth-card"><h1>Skein</h1><p>${t("signIn")}</p><form id="login-form"><input name="username" autocomplete="username" placeholder="${t("username")}" required><input name="password" type="password" autocomplete="current-password" placeholder="${t("password")}" required><button>${t("signIn")}</button></form><details><summary>Create an account</summary><form id="register-form"><input name="username" autocomplete="username" placeholder="${t("username")}" required><input name="email" type="email" autocomplete="email" placeholder="Email" required><input name="password" type="password" minlength="8" autocomplete="new-password" placeholder="${t("password")} (8+)" required><button>Register</button></form></details><div class="auth-error"></div>${languageControl()}</div></div>`);
  ...
  const showPending = session => {
    document.body.insertAdjacentHTML("beforeend",`<div class="auth-overlay"><div class="auth-card"><h2>Verify your email</h2><p>This account is pending. Enter the six-digit code sent to <b>${session.user.email||"your email"}</b>.</p><form id="verify-form"><input name="code" inputmode="numeric" pattern="[0-9]{6}" maxlength="6" placeholder="000000" required><button>Verify account</button></form><button id="resend-code">Send a new code</button><button id="pending-logout">${t("signOut")}</button><div class="auth-error"></div>${languageControl()}</div></div>`);
    const language=document.querySelector("#language-choice"); language.value=window.skeinI18n.selected; language.onchange=()=>window.skeinI18n.set(language.value);
    ...
    document.querySelector("#resend-code").onclick=async()=>{try{const result=await request("/api/auth/resend",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({language:window.skeinI18n.language})});document.querySelector(".auth-error").textContent=`New code sent. Expires in ${result.expires_in_seconds/60} minutes.`}catch(error){document.querySelector(".auth-error").textContent=error.message}};
  ...
  const renderAdmin = async session => {
    ...
    if (can("users.manage")) {
      ...
      document.querySelector("#rbac-users").innerHTML = `<h3>User and profile management</h3><form id="create-user" class="rbac-create"><input name="username" placeholder="${t("username")}" required><input name="email" type="email" placeholder="Email (optional)"><input name="password" type="password" minlength="8" placeholder="${t("password")} (8+)" required><div class="profile-picker">${profileOptions(["workflow_operator"])}</div><button>${t("createUser")}</button></form><div id="user-table" class="user-table">${users.map(user => `<div class="rbac-user"><div><b>${user.username}</b><small>${user.email||"No email"} · ${user.verified?"VERIFIED":"PENDING"}<br>${user.permissions.join(" · ")}</small></div><div class="profile-picker" data-profiles="${user.id}">${profileOptions(user.profiles.map(profile=>profile.id))}</div><label><input type="checkbox" data-active="${user.id}" ${user.active?"checked":""}> ${t("active")}</label><input type="password" data-password="${user.id}" minlength="8" placeholder="New password"><div><button data-save-user="${user.id}">Save</button>${!user.verified&&can("users.verify")?`<button data-approve-user="${user.id}">Approve</button>`:""}</div></div>`).join("")}</div>`;
    ...
    if(can("email.manage")){
      ...
      document.querySelector("#smtp-settings").innerHTML=`<h3>Outbound email server</h3><form id="smtp-form" class="smtp-form"><input name="host" placeholder="SMTP host" value="${config.host||""}" required><input name="port" type="number" value="${config.port||587}" required><select name="security"><option value="starttls" ${config.security==="starttls"?"selected":""}>STARTTLS</option><option value="ssl" ${config.security==="ssl"?"selected":""}>SSL/TLS</option><option value="plain" ${config.security==="plain"?"selected":""}>Plain</option></select><input name="username" placeholder="SMTP username" value="${config.username||""}"><input name="password" type="password" placeholder="SMTP password (unchanged if empty)"><input name="from_address" type="email" placeholder="Sender address" value="${config.from_address||""}" required><button>Save SMTP</button></form><form id="smtp-test" class="smtp-test"><input name="recipient" type="email" placeholder="Test recipient" required><button>Send test email</button></form><div id="smtp-status"></div>`;
      document.querySelector("#smtp-form").onsubmit=async event=>{event.preventDefault();try{await request("/api/admin/email",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(Object.fromEntries(new FormData(event.target)))});document.querySelector("#smtp-status").textContent="SMTP configuration saved."}catch(error){document.querySelector("#smtp-status").textContent=error.message}};
      document.querySelector("#smtp-test").onsubmit=async event=>{event.preventDefault();try{await request("/api/admin/email/test",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(Object.fromEntries(new FormData(event.target)))});document.querySelector("#smtp-status").textContent="Test email sent."}catch(error){document.querySelector("#smtp-status").textContent=error.message}};
    }
  ...
  const boot = async () => {
    try {
      ...
      const script=document.createElement("script"); script.src="/app.js?v=3"; document.body.appendChild(script);
